...
- [x] **CFG-01**: Config directory is `~/.config/fenec` on all platforms (macOS switches from `~/Library/Application Support/fenec`)
- [x] **CFG-02**: Existing data auto-migrates from `~/Library/Application Support/fenec` to `~/.config/fenec` on macOS first run
- [x] **CFG-03**: User sees migration feedback message on stderr after successful migration

### Conversation

- [x] **CONV-01**: User can type `/clear` in REPL to reset conversation to initial state
- [x] **CONV-02**: Previous session auto-saves to named file before clear (no data loss)
- [x] **CONV-03**: System prompt and tool descriptions preserved after clear (tools remain functional)
