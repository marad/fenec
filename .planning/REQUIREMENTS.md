...
- [ ] **COPILOT-01**: User can add a `copilot` provider with only `type = "copilot"` in TOML — no url or api_key fields required
- [ ] **COPILOT-02**: Provider resolves auth token automatically: `GH_TOKEN` env var → `GITHUB_TOKEN` env var → `gh auth token` subprocess
- [ ] **COPILOT-03**: Provider returns a clear, actionable error if `gh` CLI is not installed or the user is not authenticated
- [ ] **COPILOT-04**: Chat completions and streaming work via `https://models.github.ai/inference` using the existing openai-go/v3 SDK
- [ ] **COPILOT-05**: Tool calling works identically to the existing `openai` provider (same format, no changes to tool system)
...
- [ ] **COPILOT-09**: Default model for the `copilot` provider is `gpt-4o-mini` (Copilot Free compatible, supports tool calling)
...
|-------------|-------|--------|
| COPILOT-01 | Phase 12 | Pending |
| COPILOT-02 | Phase 12 | Pending |
| COPILOT-03 | Phase 12 | Pending |
| COPILOT-04 | Phase 12 | Pending |
| COPILOT-05 | Phase 12 | Pending |
| COPILOT-06 | Phase 13 | Pending |
| COPILOT-07 | Phase 13 | Pending |
| COPILOT-08 | Phase 13 | Pending |
| COPILOT-09 | Phase 12 | Pending |
